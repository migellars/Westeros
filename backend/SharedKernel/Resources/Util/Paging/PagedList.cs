...
namespace SharedKernel.Resources.Util.Paging
{
    public class PagedList<T>
    {
        ...
        private sealed class PagedSearchQueryBuilder : IPagedQueryBuilder<T>
        {
            private IQueryable<T> _query;
            private readonly PagedList<T> _paged;
            private string _searchTerm;

            public PagedSearchQueryBuilder(IQueryable<T> query, PagedList<T> pagedList, string searchTerm = "")
            {
                _query = query;
                _paged = pagedList;
            ...
            public IPagedQueryBuilder<T> Order<TKey>(Expression<Func<T, TKey>> keySelector, string order)
            {
                _query = order == PageOrder.DESC.Name
